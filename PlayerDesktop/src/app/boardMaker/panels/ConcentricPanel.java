...
public class ConcentricPanel extends JPanel implements ItemListener
{
	...
	public ConcentricPanel(JDialog dialog, Maker maker) {
		...
		sideSpinner = new JSpinner(new SpinnerNumberModel(1,1,Integer.MAX_VALUE,1));
	...
	private JButton makeCreateButton() {
		...
		button.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				{
				case 0:
					maker.setGraphFunction(Concentric.construct((ConcentricShapeType)shapes.getSelectedItem(), null, null, new DimConstant((int)ringSpinner.getValue()), 
							((int)stepSpinner.getValue() == 0) ? null : new DimConstant((int) stepSpinner.getValue()), bf0, bf1, bf2, bf3));
					dialog.dispose();
					break;
				case 1:
					maker.setGraphFunction(Concentric.construct(null, new DimConstant((int)sideSpinner.getValue()), null, new DimConstant((int)ringSpinner.getValue()), 
							((int)stepSpinner.getValue() == 0) ? null : new DimConstant((int) stepSpinner.getValue()), bf0, bf1, bf2, bf3));
					dialog.dispose();
					break;
				case 2:
					if (isCorrectFormat(dimensions.getText())) {
						maker.setGraphFunction(Concentric.construct(null, null, parseDimensions(), new DimConstant((int)ringSpinner.getValue()), 
								((int)stepSpinner.getValue() == 0) ? null : new DimConstant((int) stepSpinner.getValue()), bf0, bf1, bf2, bf3));
						dialog.dispose();
					} else {
						// dialog to reformat
